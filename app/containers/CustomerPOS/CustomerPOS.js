...
const mnemonic = BITBOX
  .Mnemonic
  .generate(256, BITBOX.Mnemonic.wordLists()[lang]);

// root seed buffer
const rootSeed = BITBOX
  .Mnemonic
  .toSeed(mnemonic);

// master HDNode
const masterHDNode = BITBOX
  .HDNode
  .fromSeed(rootSeed, 'bitcoincash');

// extended public key
const xpubkey = BITBOX
  .HDNode
  .toXPub(masterHDNode);

// HDNode of BIP44 account
const account = BITBOX
  .HDNode
...
export default class CustomerPOS extends Component {
  ...
  render() {
    ...
    for (let i = 0; i < 10; i++) {
      const account = masterHDNode.derivePath(`m/44'/145'/0'/0/${i}`);
      addresses.push(
        <li key={i}>
