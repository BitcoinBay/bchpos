...
CCC.TRADE.unpack = function (tradeString) {
  let valuesArray = tradeString.split('~');
...
CCC.CURRENT.unpack = function (value) {
  ...
  for (let property in this.FIELDS) {
    if (this.FIELDS[property] === 0) {
      ...
      // i know this is a hack, for cccagg, future code please don't hate me:(, i did
      // this to avoid subscribing to trades as well in order to show the last market
...
CCC.CURRENT.getKeyFromStreamerData = function (streamerData) {
  let valuesArray = streamerData.split('~');
...
CCC.filterNumberFunctionPolyfill = function (value, decimals) {
  ...
  let parts = numberWithCorrectDecimals
    .toString()
    .split('.');
